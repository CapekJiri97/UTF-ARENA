...
export class Player{
  ...
  update(dt){ if(game.gameOver) return;
    ...
    if (this.shieldExplodeData) {
        ...
        if (this.shieldExplodeData.timer <= 0 || this.shield <= 0) {
            ...
            for(let m of game.minions){ if(!m.dead && m.team !== this.team && dist(this.pos, m.pos) <= expl.radius){ applyDamage(m, expl.damage, expl.dmgType, this.id); spawnParticles(m.pos.x, m.pos.y, 4, '#fff'); if(m.hp<=0){ m.dead = true; if (!socket || game.isHost) { this.gold += 10; this.totalGold += 10; this.exp += 15; this.totalExp = (this.totalExp||0) + 15; } } } }
    ...
    if (this.dashTimer > 0) {
        ...
        if (this.dashTimer <= 0 && this.dashEndExplosion) {
           ...
           for(let m of game.minions){ if(!m.dead && m.team !== this.team && dist(this.pos, m.pos) <= range){ applyDamage(m, expl.damage, expl.dmgType, expl.id); spawnParticles(m.pos.x, m.pos.y, 4, '#fff'); if(m.hp<=0){ m.dead = true; if (!socket || game.isHost) { this.gold += 10; this.totalGold += 10; this.exp += 15; this.totalExp = (this.totalExp||0) + 15; } } } }
  ...
  castSpell(spKey, targetX, targetY, isNetwork = false){ 
    ...
    if (sp.type === 'projectile') {
        ...
        for(let m of game.minions){ if(!m.dead && m.team !== this.team && dist(this.pos, m.pos) <= range){ applyDamage(m, damage, this.dmgType, this.id); spawnParticles(m.pos.x, m.pos.y, 4, '#fff'); if(m.hp<=0){ m.dead = true; if (!socket || game.isHost) { this.gold += 10; this.totalGold += 10; this.exp += 15; this.totalExp = (this.totalExp||0) + 15; } } } } 
        ...
        for(let m of game.minions){ if(!m.dead && m.team !== this.team && dist(this.pos, m.pos) <= range){ 
            ...
            if(m.hp<=0){ m.dead = true; if (!socket || game.isHost) { this.gold += 10; this.totalGold += 10; this.exp += 15; this.totalExp = (this.totalExp||0) + 15; } } 
        } } 
...
export class BotPlayer extends Player {
    ...
    evaluateStrategy() {
      ...
      // Levelování spellů
    if (!socket || game.isHost) { // Pouze Host smí levelovat spelly
        while(this.spellPoints > 0) {
            this.allocateSpellPoint(Math.random() > 0.5 ? 'Q' : 'E');
        }
    }

    if (!this.alive) return; // Mrtvý bot nic nedělá

      // Globální zhodnocení mapy (Prohráváme?)
      ...
      this.isGlobalLosing = myTowersCount < enemyTowersCount;
    ...
    evaluateTactic() {
      ...
      for (let m of activeAllyMinions) {
              ...
              for (let otherM of activeAllyMinions) { if (dist(m.pos, otherM.pos) < 350) nearbyMinions++; }
              ...
              if (h.active) {
                  ...
                  for (let id in game.teamIntents[this.team]) {
                      ...
                      if (intent && intent.state === 'PICKUP' && intent.objective && intent.objective.type === 'heal' && intent.objective.pos.x === h.pos.x) {
                          ...
                          if (ally && dist(ally.pos, h.pos) < dist(this.pos, h.pos)) { allyGoingForHeal = true; break; }
                      }
                  }
      ...
      if (game.powerup && game.powerup.active && !this.hasPowerup) {
          ...
          for (let id in game.teamIntents[this.team]) {
              ...
              if (intent && intent.state === 'PICKUP' && intent.objective && intent.objective.type === 'powerup') {
                  let ally = aliveAllies.find(p => p.id === id);
                  if (ally && (dist(ally.pos, game.powerup.pos) < dist(this.pos, game.powerup.pos) || dist(ally.pos, game.powerup.pos) < 150)) { 
      ...
      for (let e of enemies) {
          ...
          if (d < this.personalWeights.attackVisionRange || this.huntTarget === e || (isBloodlust && this.target === e)) {
              ...
              if (e.className && this.huntTarget !== e) {
                  let allyStrength = 0;
                  let enemyStrength = 0;
                  for (let p of aliveAllies) { if (dist(p.pos, e.pos) < 600) allyStrength += Math.max(0, p.hp / (p.effectiveMaxHp || p.maxHp)); }
                  for (let p of aliveEnemies) { if (dist(p.pos, e.pos) < 600) enemyStrength += Math.max(0, p.hp / (p.effectiveMaxHp || p.maxHp)); }
    ...
    update(dt) {
      ...
      if(!this.alive){
          ...
          // Mrtvý bot stále nakupuje
          this.strategyTimer -= dt; 
          if (this.strategyTimer <= 0) { this.strategyTimer = 1.0; this.evaluateStrategy(); } // Nakupuje mrtvý
          return; 
      }
      ...
      // --- Zpracování vrstev v čase ---
      this.strategyTimer -= dt;
      if (this.strategyTimer <= 0) { this.strategyTimer = 2.0; this.evaluateStrategy(); }
