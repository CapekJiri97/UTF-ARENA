...
export class Player{
  ...
  update(dt){ if(game.gameOver) return;
    ...
    if (this.dashTimer > 0) {
        ...
        if (this === player) { // PŘIDÁNO: Zabráníme aplikaci lokálních WASD na cizí hráče
            ...
            // Plynulá interpolace (Klouzání směrem k serveru místo skákání)
  ...
  castSpell(spKey, targetX, targetY, isNetwork = false){ 
    ...
    if (sp.type === 'projectile') {
      ...
      for(let p of game.players){ if(p !== this && p.team !== this.team && p.alive && dist(this.pos, p.pos) <= range){ applyDamage(p, damage, this.dmgType, this.id); spawnParticles(p.pos.x, p.pos.y, 4, '#fff'); if(p.hp<=0){ handlePlayerKill(p, this.id); } } } 
